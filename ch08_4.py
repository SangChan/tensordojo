...
def get_outputs(image):
    ...
    return [gram_matrix(out) for out in output]

@tf.function()
def train_step(image):
    with tf.GradientTape() as tape:
        _outputs = get_outputs(image)
        _output2 = get_content_output(image)
        loss = style_weight * get_loss(_outputs, style_outputs)
        loss += content_weight * get_content_loss(_output2, content_output)
